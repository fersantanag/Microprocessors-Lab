...
Textlabel TX_Info,TX_Info1,TX_Info2,TX_Info3;
...
boolean toggleServo=false,toggleWriteServo=false,toggleWing=false,toggleWriteWing=false,toggleLive=false,toggleWriteServoLive=false,toggleWriteWingLive=false,
	...
	graphEnabled = false,Mag_=false,gimbal=false, servoStretch=false,camTrigger=false,ExportServo=false,
        toggleTrigger=false,ServosActive=false,ToggleJoystick=false;;

static int RCThro = 3, RCRoll = 0, RCPitch =1, RCYaw =2, RCAUX1=4, RCAUX2=5, RCAUX3=6, RCAUX4=7;

// For USE_MSP_WP
  int  wp_radius, safe_wp_distance, nav_max_altitude, nav_speed_max, nav_speed_min, nav_bank_max, rth_altitude,fence;       // in cm/s
  int  crosstrack_gain, land_speed, max_wp_number;
   int filtering,lead_filter ,dont_reset_home_at_arm , nav_controls_heading , nav_tail_first , nav_rth_takeoff_heading;
   int slow_nav, wait_for_rth_alt ,ignore_throttle,takeover_baro;
int  gpsByte1, gpsByte2 ;
...
Button buttonIMPORT, buttonSAVE, buttonREAD, buttonRESET, buttonWRITE, buttonCALIBRATE_ACC, buttonCALIBRATE_MAG, buttonSTART, buttonSTOP, buttonSETTING,
       ...
       btMagDecl,btMWiiHome,btDownloads,btnRCJoystick;
...
Toggle tACC_ROLL, tACC_PITCH, tACC_Z, tGYRO_ROLL, tGYRO_PITCH, tGYRO_YAW, tBARO,tHEAD, tMAGX, tMAGY, tMAGZ,
        tDEBUG1, tDEBUG2, tDEBUG3, tDEBUG4, modeToggle;
...
CheckBox checkbox[], checkboxRev[], GPSboxes[];
CheckBox Bbox,  Wbox,  Mbox, GPSbox;
Button buttonCheckbox[], BtServo[], BtAUX[];
Numberbox RateSlider[];
Slider   GpsSlider[];

public void create_GPStab(){
  GPSboxes = new CheckBox[10];
  GpsSlider = new Slider[11];

  GPSbox = controlP5.addCheckBox("GPSbox").setPosition(xServ+100,yServ+10).setColorForeground(color(120))
                ...
                .addItem("Enable GPS filtering", 254).addItem("GPS forward prediction filter", 254).addItem("Don't reset home at arm", 254)
                .addItem("Nav controlls heading", 254).addItem("Fly with tail first", 254).addItem("Turn takeoff hesding at mone", 254)
                .addItem("Wailt for RTH alt", 254).addItem("Enable slow navigation", 254).addItem("ingnore thr during nav", 254)
                .addItem("Take over Baro mode", 254)
               // .hide().hideLabels()
               .moveTo("GPS")
                ;
                
  GpsSlider[0]  =    controlP5.addSlider("wp_radius",0,+1000,0,xServ-200,yServ+10,50,10).setDecimalPrecision(1).setLabel(" WP radius") .moveTo("GPS");
  GpsSlider[1]  =    controlP5.addSlider("safe_wp_distance",0,+3000,0,xServ-200,yServ+30,50,10).setDecimalPrecision(1).setLabel(" Max wp distance") .moveTo("GPS");
  GpsSlider[2]  =    controlP5.addSlider("nav_max_altitude",0,+500,0,xServ-200,yServ+50,50,10).setDecimalPrecision(1).setLabel(" Max Nav Altitude(m)") .moveTo("GPS");
  GpsSlider[3]  =    controlP5.addSlider("nav_speed_max",0,+1000,0,xServ-200,yServ+70,50,10).setDecimalPrecision(1).setLabel(" Max navspeed(cm/s)") .moveTo("GPS");
  GpsSlider[4]  =    controlP5.addSlider("nav_speed_min",0,+1000,0,xServ-200,yServ+90,50,10).setDecimalPrecision(1).setLabel(" Min navspeed(cm/s)") .moveTo("GPS");
  GpsSlider[5]  =    controlP5.addSlider("crosstrack_gain",0,+200,0,xServ-200,yServ+110,50,10).setDecimalPrecision(1).setLabel(" Crosstrack gain") .moveTo("GPS");
  GpsSlider[6]  =    controlP5.addSlider("nav_bank_max",0,+45,0,xServ-200,yServ+130,50,10).setDecimalPrecision(1).setLabel(" Max Nav bank(deg)") .moveTo("GPS");
  GpsSlider[7]  =    controlP5.addSlider("rth_altitude",0,+1000,0,xServ-200,yServ+150,50,10).setDecimalPrecision(1).setLabel(" RTH altitude(m)") .moveTo("GPS");
  GpsSlider[8]  =    controlP5.addSlider("land_speed",0,+500,0,xServ-200,yServ+170,50,10).setDecimalPrecision(1).setLabel(" Land speed") .moveTo("GPS");
  GpsSlider[9]  =    controlP5.addSlider("Fence_radius",0,+1000,0,xServ-200,yServ+190,50,10).setDecimalPrecision(0).setLabel(" Fence radius(m)") .moveTo("GPS");
  GpsSlider[10]  =    controlP5.addSlider("max_wp_number",0,+254,0,xServ-200,yServ+210,50,10).setDecimalPrecision(1).setLabel(" Max Wp") .moveTo("GPS");
}
...
if(multiType == PPM_TO_SERVO || multiType == AIRPLANE ){
  controlP5.getTab("ServoSettings").show();
   ...
   //TxtRev2    = controlP5.addTextlabel("txtlblRev","bit2").setPosition(xServ+20,yServ+20).hide().moveTo("ServoSettings").hide();
...
public void setup() {
  ...
  TX_Info   = controlP5.addTextlabel("TxInf0","This Emulates a normal 8ch Transmitter with selectable mode.") .setPosition(xServ-100, yServ+60).moveTo("ser_TX");
  TX_Info1  = controlP5.addTextlabel("TxInf1","Only safe for BenchTesting!!") .setPosition(xServ-100, yServ+80).moveTo("ser_TX");
  TX_Info2  = controlP5.addTextlabel("TxInf2","Throttle have a safty lock that must be relesed") .setPosition(xServ-100, yServ+100).moveTo("ser_TX");
  TX_Info3  = controlP5.addTextlabel("TxInf3","Warning!!.. \nRemove propellors For safty. Motors can start accidentally\n\nUse with Causion!\nPress start to enable TransmitterMode!") .setPosition(xServ-100, yServ+140).moveTo("ser_TX");
...
"Select motor(s) and control throttle.").setPosition(xServ-200, yServ+10).moveTo("Motors");
  ...
  for( i=0;i<2;i++) {
    confPow[i] = (controlP5.Numberbox) hideLabel(controlP5.addNumberbox("confPow"+i,0,xParam+250,yParam+10+i*35,60,14));
    confPow[i].setColorBackground(green_).setMin(0).setDirection(Controller.HORIZONTAL).setDecimalPrecision(0).setMultiplier(1).setMax(100).moveTo("Config");//.hide()
  }
  confPow[0].setLabel("Pint2ma").setMin(0).setMax(512).moveTo("Config");
  confPow[1].setLabel("Psensornull").setMin(0).setMax(2000).moveTo("Config");
  ...
  confINF[6].setLabel("MAG Decl").setMin(-50).setMax(50).setDecimalPrecision(1).setMultiplier(.1f).moveTo("Config");
  ...
  btnRCJoystick =       controlP5.addButton("JOYSTICK" ,  1,  xGraph+200, yGraph-280,110,19) .setLabel("       Start").setColorBackground(red_).moveTo("ser_TX");
  modeToggle    =       controlP5.addToggle("TX_MODE",true, xGraph+350, yGraph-300,60,19).moveTo("ser_TX").setCaptionLabel("Mode 2").setColorBackground(orange_);
...
private static final int
  ...
  MSP_ACC_TRIM             =240,    //  get/set acc angle trim values
...
public void evaluateCommand(byte cmd, int dataSize) {
  ...
  switch(icmd) {
    ...
    case MSP_NAV_CONFIG:
      gpsByte1  =read8();
      gpsByte2  =read8();     
     if ((gpsByte1&1) >0){GPSbox.activate(0);}else {GPSbox.deactivate(0);}// Filtering
     if ((gpsByte1&2) >0){GPSbox.activate(1);}else {GPSbox.deactivate(1);}// Prediction Filtering
     if ((gpsByte1&4) >0){GPSbox.activate(2);}else {GPSbox.deactivate(2);}// dont_reset_home_at_arm 
     if ((gpsByte1&8) >0){GPSbox.activate(3);}else {GPSbox.deactivate(3);}// nav_controls_heading
     if ((gpsByte1&16)>0){GPSbox.activate(4);}else {GPSbox.deactivate(4);}// nav_tail_first 
     if ((gpsByte1&32) >0){GPSbox.activate(5);}else{GPSbox.deactivate(5);}// nav_rth_takeoff_heading
     if ((gpsByte1&64) >0){GPSbox.activate(7);}else{GPSbox.deactivate(7);}// slow_nav 
     if ((gpsByte1&128)>0){GPSbox.activate(6);}else{GPSbox.deactivate(6);}// wait_for_rth_alt 
     if ((gpsByte2&1) >0) {GPSbox.activate(8);}else{GPSbox.deactivate(8);}// ignore_throttle 
     if ((gpsByte2&2) >0){GPSbox.activate(9);}else {GPSbox.deactivate(9);}// takeover_baro 
     GpsSlider[0].setValue( wp_radius  =read16());
     GpsSlider[1].setValue( safe_wp_distance  =read16());
     GpsSlider[2].setValue( nav_max_altitude  =read16());
     GpsSlider[3].setValue( nav_speed_max =  read16()); //nav_speed_max
     GpsSlider[4].setValue( nav_speed_min  =read16());
     GpsSlider[5].setValue( crosstrack_gain  =read8());
     nav_bank_max  =read16();
     GpsSlider[6].setValue(nav_bank_max/100 );
     GpsSlider[7].setValue( rth_altitude  =read16());
     GpsSlider[8].setValue( land_speed  =read8());
     GpsSlider[9].setValue( fence  =read16());
     GpsSlider[10].setValue( max_wp_number  =read8());
      break;
...
//         println( read16());
    println("####################"); break;
    case MSP_CONFIG:
        confPow[0].setValue(read8()).show(); 
        confPow[1].setValue(read16()).show();
       break;
    case MSP_COMP_GPS:
...
public void draw() {
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if (!toggleRead && !toggleWrite && !toggleSetSetting ) {
      ...
      if ((time-time6)>200 ) {
        time6=time;
        int[] requests = {MSP_RAW_GPS, MSP_COMP_GPS, MSP_ANALOG}; // MSP_PIPAK
        sendRequestMSP(requestMSP(requests));
   ...
   if ((time-time7)>20 && ToggleJoystick ){time7=time; SendJoystick();}
    }
    if (toggleReset) {
      toggleReset=false;
      toggleRead=true;
...
/*****************************************************************/
    ...
    if (toggleRead) {
      if (!toggleLive &&( ActiveTab=="default" ||  ActiveTab =="Config" || ActiveTab =="ser_TX" || ActiveTab =="GPS")){// && ActiveTab=="default".. Because of checksum error on MSP_PID with servosTab
        int[] requests = {MSP_IDENT ,MSP_BOXNAMES, MSP_RC_TUNING, MSP_PID, MSP_MOTOR_PINS,MSP_BOX,MSP_MISC,MSP_NAV_CONFIG,MSP_CONFIG};  // MSP_PIDNAMES, MSP_SERVO_CONF
        sendRequestMSP(requestMSP(requests));
      }
...
//MSP_SET_CONFIG
      payload = new ArrayList<Character>();
      int qm = round(confPow[0].value());payload.add(PApplet.parseChar(qm)); // pint2ma
      qm = (int)confPow[1].value(); payload.add(PApplet.parseChar (qm % 256) ); payload.add(PApplet.parseChar (qm / 256)  ); // psensornull
      sendRequestMSP(requestMSP(MSP_SET_CONFIG,payload.toArray( new Character[payload.size()]) )); // Send settings
  ...
  text(pMeterSum * 1.0f + " mA", xGraph +50, yGraph -35);
...
//  .setColorBackground(color(blue_))
  ...
  .hide()
  ;
    controlP5.addTab("ser_TX")
  .activateEvent(true)
//  .setColorBackground(color(blue_))
  .setColorLabel(color(255))
  //.setColorActive(color(255,128,0))
  .setColorActive(color(green_))
  .setLabel("Transmitter")
  .setId(5)
  //.hide()
  ;
    controlP5.addTab("GPS")
  .activateEvent(true)
//  .setColorBackground(color(blue_))
  .setColorLabel(color(255))
  //.setColorActive(color(255,128,0))
  ...
  int tHeight = 20;
  controlP5.window().setPositionOfTabs(120, 6);
  if( pVersion == 1.5f ) {
    tHeight = 17;
  }
  ...
  controlP5.tab("ser_TX").setHeight(tHeight);
 }
...
public void TX_MODE(boolean theFlag) {
...
public void  JOYSTICK(){ // Toggle joystick Activation
...
RightStick.setArrayValue(new float[] {rightX-1000,rightY-2000});
      ...
      List<Character> payload;
      payload = new ArrayList<Character>();
      for (i=0;i<8;i++){payload.add(PApplet.parseChar((int)Joy[i] % 256));payload.add(PApplet.parseChar((int)Joy[i] / 256));}
