...
"Select motor(s) and control throttle.").setPosition(xServ-200, yServ+10).moveTo("Motors");
  ...
  confINF[6].setLabel("MAG Decl").setMin(-50).setMax(50).setDecimalPrecision(1).setMultiplier(.1).moveTo("Config");
...
//MSP_SET_CONFIG
      payload = new ArrayList<Character>();
      int qm = round(confPow[0].value());payload.add(char(qm)); // pint2ma
      qm = (int)confPow[1].value(); payload.add(char (qm % 256) ); payload.add(char (qm / 256)  ); // psensornull
