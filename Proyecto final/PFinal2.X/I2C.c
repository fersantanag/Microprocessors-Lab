...
void I2C1GeneraStart(void) {
    I2C1CONbits.SEN = 1; // Genera condici?n de start en el bus
    while (IFS1bits.I2C1MIF == 0); // Espera el final de la condici?n
    IFS1bits.I2C1MIF = 0; // Borro el flag
}

void I2C1GeneraReStart(void) {
    I2C1CONbits.RSEN = 1; // Genera condici?n de repeteaded
    // start en el bus
    while (IFS1bits.I2C1MIF == 0); // Espera el final de la condici?n
    IFS1bits.I2C1MIF = 0; // Borro el flag
}
...
void I2C1GeneraStop(void) {
    I2C1CONbits.PEN = 1; // Genera condici?n de stop en el bus
    while (IFS1bits.I2C1MIF == 0); // Espera el final de la condici?n
    IFS1bits.I2C1MIF = 0; // Borro el flag
}
...
uint8_t I2C1LeeByte(int ack) {
    // Lectura
    uint8_t dato;
    I2C1CONbits.RCEN = 1; // Se activa la recepci?n del dato (reloj)
    while (IFS1bits.I2C1MIF == 0); // Espera el final de la recepci?n
    IFS1bits.I2C1MIF = 0; // Borro el flag
    dato = I2C1RCV;
    // Envio ACK
    I2C1CONbits.ACKDT = ack & 1; // Bit 5 del I2CxCON
    I2C1CONbits.ACKEN = 1; // Se env?a el ACK
    while (IFS1bits.I2C1MIF == 0); // Espera el env?o del ACK
    IFS1bits.I2C1MIF = 0; // Borro el flag
    return dato;
}
...
int I2C1EscribeByte(uint8_t dato) {
    I2C1TRN = dato;
    while (IFS1bits.I2C1MIF == 0); // Espera el final del env?o
    IFS1bits.I2C1MIF = 0; // Borro el flag
